...
const gameOriginalIds = 'anpanman baikinman dokinchan shokupanman currypanman melonpanna rollpanna creampanda jamojisan batakosan'.split(' ');
...
function build() {
  ...
  // Validate every source before creating any output. Canonical originals are never written.
  const originals = new Map(ids.map(id => [id, loadOriginal(id)]));
  const tracks = require('./anpanman-score.cjs').renderTracks();
  if (!Array.isArray(tracks) || tracks.length !== gameOriginalIds.length || new Set(tracks.map(t => t?.id)).size !== gameOriginalIds.length) throw new Error('Expected ten unique game-original tracks');
  const composed = gameOriginalIds.map(id => {
    const track = tracks.find(t => t?.id === id);
    if (!track || track.rate !== RATE || !Array.isArray(track.data) || ![1, 2].includes(track.data.length) || track.data.some(c => !(c instanceof Float32Array) || c.length !== 230400) || !Number.isFinite(track.gain) || track.gain <= 0 || typeof track.title !== 'string' || !track.title.trim() || typeof track.instrument !== 'string' || !track.instrument.trim()) throw new Error(`Invalid game-original track: ${id}`);
    return { track, wav: writeWav(track) };
  });
  ...
  for (const { track, wav } of composed) {
    ...
    files.push([file, `/* Composed game-original music; NOT an official song. Generated by tools/build-audio-assets.cjs. */\nwindow.MoonAudio.registerAsset(${JSON.stringify(id)},${JSON.stringify({ data: wav.toString('base64'), type: 'audio/wav', ...metadata })});\n`]);
    composedRows.push({ id, title, instrument, source: metrics(track), normalized: metrics(readWav(wav)), tailFrames: 0, bridgeMax: 0, gain, packBytes: Buffer.byteLength(files.at(-1)[1]), wavSha256: hash(wav) });
  }
  files.push(['games-audio-manifest.js', `/* Generated. Black is one-shot only (loopable:false). See AUDIO-ASSETS.md. */\nwindow.MoonAudioManifest = ${JSON.stringify(manifest, null, 2)};\n`]);
  ...
  fs.writeFileSync(path.join(root, 'AUDIO-ASSETS.md'), documentation(rows, composedRows));
  return { manifest, rows: [...rows, ...composedRows], generatedFiles: [...files.map(([file]) => file), 'AUDIO-ASSETS.md'] };
}

function documentation(rows, composedRows) {
  const lines = [
  ...
  lines.push('', 'Black (one-shot only): RMS 0.162238; peak 0.823391; first/last >-60dB 0.332813 / 17.414917 s. After gain 0.2: RMS about 0.032448, peak about 0.164678. MP3 decoder details can vary slightly with browser version; re-run analysis for current values.', '', '## Per-Track Decisions', '');
  ...
  lines.push('', '## Anpan10 Game-Original Music', '',
  ...
  for (const r of composedRows) lines.push(`| ${r.id} | ${cell(r.title)} | ${cell(r.instrument)} | ${r.gain} | ${r.wavSha256} |`);
  ...
  lines.push('### Additional Game-Original Files', '', '- `tools/anpanman-score.cjs` (composition renderer)', ...composedRows.flatMap(r => [`- \`sounds/${r.id}.wav\``, `- \`sounds/packed/${r.id}.js\` (${r.packBytes} bytes)`]), '', `Game-original pack bytes: ${composedRows.reduce((n, r) => n + r.packBytes, 0)}. Total across all 30 packs: ${[...rows, ...composedRows].reduce((n, r) => n + r.packBytes, 0) + blackBytes}.`, '');
  return lines.join('\n');
}
